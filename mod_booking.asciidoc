...
|bookablility  |Bookable | ? | The number of charging stations that are bookable at this location and if this is required.
...
|noshow_fee           | boolean | ? | If the CPO will charge a no show fee. The amount of the fee can be defined in the booking_terms URL.
...
|===
|Property |Type |Card. |Description
|request_status | <<mod_bookings_request_status_enum, RequestStatus>> | 1 | The current state of the booking request.
...
|min_power  | number | ? | Power guaranteed/requested during this timeslot, in Watts (W).
|max_power  | number | ? | Maximum power available/requested during this timeslot, in Watts (W).
