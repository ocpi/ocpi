...
|license_plate | <<types.asciidoc#types_cistring_type,CiString>> | ? | Only possible if it is part as one of the <<mod_bookings_location_access_enum, LocationAccess>> options in the <<mod_bookings_booking_terms_class, BookingTerms>>.
...
|location_access  | <<mod_bookings_location_access_enum, LocationAccess>> |1 | The reason why the booking is canceled.
