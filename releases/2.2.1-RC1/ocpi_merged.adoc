...
** <<mod_locations_location_object,Locations>>: Multiple Tariffs, Lots of small improvements
...
|Functional Module | OCPI Module that provides functionality/information for the EV Driver, such as: <<mod_tokens_tokens_module,Tokens>>,
                     ...
                     Functional Modules use <<transport_and_format_message_routing,message routing>>.
...
These are different 'tokens' than the <<mod_tokens_token_object,Tokens>> exchanged via the <<mod_tokens_tokens_module,Token Module>>: Tokens used by drivers to authorize charging.
...
than: <<credentials_credentials_endpoint,`credentials`>>
...
The <<credentials_credentials_object,country_code's>>
and <<credentials_credentials_object,party_id's>>
of the parties on the other platform are received during the <<credentials_credentials_endpoint,credentials>> handshake
in the <<credentials_credentials_role_class,CredentialsRoles>>.
The roles exchanged during the <<credentials_credentials_endpoint,credentials>> handshake
...
than one of the <<credentials_credentials_role_class,CredentialsRoles>> given during the
<<credentials_credentials_endpoint,credentials>> handshake,
...
{<<credentials_credentials_object,country_code>>}
and {<<credentials_credentials_object,party_id>>} in the object itself.

When one or more objects, returned in the response, do not meet one of the <<credentials_credentials_role_class,CredentialsRoles>>
given during the <<credentials_credentials_endpoint,credentials>> handshake, these objects may be ignored.
...
A Hub SHALL report connected parties via the <<mod_hub_client_info_module,HubClientInfo module>>.
...
|party_id |<<types_cistring_type,CiString>>(3) |1 |ID of the CPO that 'owns' this Location (following the ISO-15118 standard).
...
|operator |<<mod_locations_businessdetails_class,BusinessDetails>> |? |Information of the operator. When not specified, the information retrieved from the <<credentials_credentials_endpoint,Credentials>> module, selected by the `country_code` and `party_id` of this Location, should be used instead.
...
|START_SESSION_CONNECTOR_REQUIRED |When a <<mod_commands_startsession_object,StartSession>> is sent to this EVSE, the MSP is required to add the optional `connector_id` field in the <<mod_commands_startsession_object,StartSession>> object.
...
There are Charge Points in the field that do not yet support OCPP 2.x. If these Charge Points have multiple connectors per EVSE, the CPO needs to know which connector to start when receiving a <<mod_commands_startsession_object,StartSession>> for the given EVSE. If this is the case, the CPO should set the `START_SESSION_CONNECTOR_REQUIRED` capability on the given EVSE.
...
|party_id |<<types_cistring_type,CiString>>(3) |1 |ID of the CPO that 'owns' this Session (following the ISO-15118 standard).
...
|auth_method |<<mod_cdrs_authmethod_enum,AuthMethod>> |1 |Method used for authentication. This might change during a session, for example when the session was started with a reservation: <<mod_commands_reservenow_object,ReserveNow>>: <<mod_cdrs_authmethod_enum,`COMMAND`>>. When the driver arrives and starts charging using a Token that is whitelisted: <<mod_cdrs_authmethod_enum, `WHITELIST`>>.
|authorization_reference |<<types_cistring_type,CiString>>(36) |? |Reference to the authorization given by the eMSP.
                                             ...
                                             <<mod_tokens_real-time_authorization,real-time authorization>>,
                                             <<mod_commands_startsession_object,StartSession>> or
                                             <<mod_commands_reservenow_object,ReserveNow>> this field SHALL contain the same value.
...
|evse_uid |<<types_cistring_type,CiString>>(36) |1 |EVSE.uid of the EVSE of this Location on which the charging session is/was happening. Allowed to be set to: <<transport_and_format_not_available,`#NA`>> when this session is created for a reservation, but no EVSE yet assigned to the driver.
|connector_id |<<types_cistring_type,CiString>>(36) |1 |Connector.id of the Connector of this Location where the charging session is/was happening. Allowed to be set to: <<transport_and_format_not_available,`#NA`>> when this session is created for a reservation, but no connector yet assigned to the driver.
...
based tariff element to another. This is also true when switch from one (<<mod_tariffs_tariffdimensiontype_enum,`TIME`>>) based tariff element  to another (<<mod_tariffs_tariffdimensiontype_enum,`TIME`>>) based tariff element, and one <<mod_tariffs_tariffdimensiontype_enum,`PARKING_TIME`>> tariff element  to another <<mod_tariffs_tariffdimensiontype_enum,`PARKING_TIME`>> based tariff element. +
...
Only the `step_size` of the tariff (<<mod_tariffs_pricecomponent_class,PriceComponent>>) after 17:00 is taken into account, for the total of the same amount for the session.
...
Example: Charging cost 0.20 euro/Wh before 17:00 and 0.27 euro/Wh after 17:00 both have a `step_size` of 500 Wh.
...
Example: Charging cost 5 euro/hour before 17:00 and 7 euro/hour after 17:00 both have a `step_size` of 10 minutes.
If a driver charges 6 minutes before 17:00 and 22 minutes after 17:00: a total of 28 minutes charging. The `step_size` rounds this up to 30 minutes total, so 24 minutes after 17:00 will be billed.
...
and <<mod_tariffs_tariffdimensiontype_enum,`PARKING_TIME`>> Tariff Elements are both used, `step_size` is only taken into account for the total parking duration` +
...
|party_id |<<types_cistring_type,CiString>>(3) |1 |ID of the CPO that 'owns' this CDR (following the ISO-15118 standard).
|id |<<types_cistring_type,CiString>>(39) |1 |Uniquely identifies the CDR, the ID SHALL be unique per `country_code`/`party_id` combination. This field is longer than the usual 36 characters to allow for credit CDRs to have <<mod_cdrs_credit_cdrs,something appended>> to the original ID. Normal (non-credit) CDRs SHALL only have an ID with a maximum length of 36.
...
|cdr_token |<<mod_cdrs_cdr_token_object,CdrToken>> |1 |Token used to start this charging session, including all the relevant information to identify the unique token.
|auth_method |<<mod_cdrs_authmethod_enum,AuthMethod>> |1 |Method used for authentication. Multiple <mod_cdrs_authmethod_enum,AuthMethods>> are possible during a charging sessions, for example when the session was started with a reservation: <<mod_commands_reservenow_object,ReserveNow>>: <<mod_cdrs_authmethod_enum,`COMMAND`>>. When the driver arrives and starts charging using a Token that is whitelisted: <<mod_cdrs_authmethod_enum, `WHITELIST`>>. The last method SHALL be used in the CDR.
|authorization_reference |<<types_cistring_type,CiString>>(36) |? |Reference to the authorization given by the eMSP.
                                             When the eMSP provided an `authorization_reference` in either:
                                             <<mod_tokens_real-time_authorization,real-time authorization>>,
                                             <<mod_commands_startsession_object,StartSession>> or
...
|city |<<types_string_type,string>>(45) |1 |City or town.
|postal_code |<<types_string_type,string>>(10) |? |Postal code of the location, may only be omitted when the location has no postal code: in some countries charging locations at highways don't have postal codes.
|state |<<types_string_type,string>>(20) |? |State    only to be used when relevant.
|country |<<types_string_type,string>>(3) |1 |ISO 3166-1 alpha-3 code for the country of this location.
|coordinates |<<mod_locations_geolocation_class,GeoLocation>> |1 |Coordinates of the location.
|evse_uid |<<types_cistring_type,CiString>>(36) |1 |Uniquely identifies the EVSE within the CPO's platform (and suboperator platforms). For example a database unique ID or the actual _EVSE ID_. This field can never be changed, modified or renamed. This is the _technical_ identification of the EVSE, not to be used as _human readable_ identification, use the field: `evse_id` for that.  Allowed to be set to: <<transport_and_format_not_available,`#NA`>> when this CDR is created for a reservation that never resulted in a charging session.
|evse_id |<<types_cistring_type,CiString>>(48) |1 |Compliant with the following specification for EVSE ID from "eMI3 standard version V1.0" (http://emi3group.com/documents-links/[http://emi3group.com/documents-links/]) "Part 2: business objects.". Allowed to be set to: <<transport_and_format_not_available,`#NA`>> when this CDR is created for a reservation that never resulted in a charging session.
|connector_id |<<types_cistring_type,CiString>>(36) |1 |Identifier of the connector within the EVSE. Allowed to be set to: <<transport_and_format_not_available,`#NA`>> when this CDR is created for a reservation that never resulted in a charging session.
|connector_standard |<<mod_locations_connectortype_enum,ConnectorType>> |1 |The standard of the installed connector. When this CDR is created for a reservation that never resulted in a charging session, this field can be set to any value and should be ignored by the Receiver.
|connector_format |<<mod_locations_connectorformat_enum,ConnectorFormat>> |1 |The format (socket/cable) of the installed connector. When this CDR is created for a reservation that never resulted in a charging session, this field can be set to any value and should be ignored by the Receiver.
|connector_power_type |<<mod_locations_powertype_enum,PowerType>> |1 | When this CDR is created for a reservation that never resulted in a charging session, this field can be set to any value and should be ignored by the Receiver.
|===
...
|public_key              | <<types_string_type,string>>(512)     |? |Public key used to sign the data, base64 encoded.
|signed_values           | <<mod_cdrs_signed_value_class,SignedValue>>          |+ |One or more signed values.
|url                     | <<types_cistring_type,string>>(512)   |? |URL that can be shown to an EV driver. This URL gives the EV driver the possibility to check the signed data from a charging session.
|===
...
|plain_data  |<<types_string_type,string>>(512) |1 |The un-encoded string of data. The format of the content depends on the EncodingMethod field.
|signed_data |<<types_string_type,string>>(5000) |1 |Blob of signed data, base64 encoded. The format of the content depends on the EncodingMethod field.
|===
...
|country_code |<<types_cistring_type,CiString>>(2) |yes |Country code of the CPO performing the GET request on the eMSP's system.
|party_id |<<types_cistring_type,CiString>>(3) |yes |Party ID (Provider ID) of the CPO performing the GET request on the eMSP's system.
...
NOTE: Before deleting a Tariff object, it is RECOMMENDED to ensure that the Tariff object is not referenced by any <<mod_locations_connector_object,Connector object>> within the `tariff_ids`.
...
|party_id |<<types_cistring_type,CiString>>(3) |1 |ID of the CPO that 'owns' this Tariff (following the ISO-15118 standard).
...
|party_id |<<types_cistring_type,CiString>>(3) |1 |ID of the eMSP that 'owns' this Token (following the ISO-15118 standard).
|uid |<<types_cistring_type,CiString>>(36) |1 |Unique ID by which this Token, combined with the Token type, can be identified. +
...
In such case the CPO only does an <<mod_tokens_real-time_authorization,realtime authorization>>
...
|evse_uid |<<types_cistring_type,CiString>>(36) |? |Optional EVSE.uid of the EVSE of this Location on which a session is to be started. Required when `connector_id` is set.
|connector_id |<<types_cistring_type,CiString>>(36) |? |Optional Connector.id of the Connector of the EVSE on which a session is to be started. This field is required when the capability: <<mod_locations_capability_enum,START_SESSION_CONNECTOR_REQUIRED>> is set on the EVSE.
|authorization_reference |<<types_cistring_type,CiString>>(36) |? |Reference to the authorization given by the eMSP, when given,
                                             this reference will be provided in the relevant <<mod_sessions_session_object,Session>>
                                             and/or <<mod_cdrs_cdr_object,CDR>>.
|===
...
|response_url |<<types_url_type,URL>> |yes |URL that the <<mod_charging_profiles_active_charging_profiles_result_object,ActiveChargingProfileResult>> POST should be send to. This URL might contain an unique ID to be able to distinguish between GET ActiveChargingProfile requests.
|===
...
|session_id |<<types_cistring_type,CiString>>(36) |yes |The unique id that identifies the session in the CPO platform.
|response_url |<<types_url_type,URL>> |yes |URL that the <<mod_charging_profiles_clear_profiles_result_object,ClearProfileResult>> POST should be send to. This URL might contain an unique ID to be able to distinguish between DELETE ChargingProfile requests.
|===
