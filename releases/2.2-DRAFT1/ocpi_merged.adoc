...
- <<mod_commands_cancelreservation_object,Cancel Reservation>>
- Improvements on: <<mod_cdrs_cdr_object,CDRs>>/
<<mod_sessions_session_object,Sessions>>/
<<mod_cdrs_cdr_object,Locations>>/
<<mod_tokens_token_object,Tokens>>
...
|token |<<types_string_type,string>>(64) |1 |The token for the other party to authenticate in your system. 
...
|party_id |<<types_string_type,string>>(3) |1 |CPO or eMSP ID of this party. (following the 15118 ISO standard). 
|country_code |<<types_string_type,string>>(2) |1 |Country code of the country this party is operating in. 
|===

The `party_id` and `country_code` are provided here to inform a server about the `party_id` and `country_code` a client will use when pushing <<transport_and_format_client_owned_object_push,client owned objects>>. This helps a server determine the URLs a client will use when pushing a <<transport_and_format_client_owned_object_push,client owned object>>.
The `country_code` is added the make certain the URL used when pushing a <<transport_and_format_client_owned_object_push,client owned object>> is unique, there might be multiple parties in the world with the same `party_id`, but the combination should always be unique.
...
When the Server connects back to the client during the credentials registration, it might encounter problems. When this happens, the Server should add the status code: <<status_codes_3xxx_server_errors,3001>> in the response to the POST from the client. 
...
In case the server, receiving the request from a client, cannot find the endpoints it expects, then it is expected to respond to the request with a status code: <<status_codes_3xxx_server_errors,3003>>.
...
|location_id |<<types_string_type,string>>(39) |yes |Location.id of the Location object to retrieve. 
|evse_uid |<<types_string_type,string>>(39) |no |Evse.uid, required when requesting an EVSE or Connector object. 
|connector_id |<<types_string_type,string>>(36) |no |Connector.id, required when requesting a Connector object. 
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the CPO requesting this PUT to the eMSP system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the CPO requesting this PUT to the eMSP system. 
|location_id |<<types_string_type,string>>(39) |yes |Location.id of the Location object to retrieve. 
|evse_uid |<<types_string_type,string>>(39) |no |Evse.uid, required when requesting an EVSE or Connector object. 
|connector_id |<<types_string_type,string>>(36) |no |Connector.id, required when requesting a Connector object. 
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the CPO requesting this PUT to the eMSP system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the CPO requesting this PUT to the eMSP system. 
|location_id |<<types_string_type,string>>(39) |yes |Location.id of the new Location object, or the Location of which an EVSE or Location object is send 
|evse_uid |<<types_string_type,string>>(39) |no |Evse.uid, required when an EVSE or Connector object is send/replaced. 
|connector_id |<<types_string_type,string>>(36) |no |Connector.id, required when a Connector object is send/replaced. 
|===
...
|id |<<types_string_type,string>>(39) |1 |Uniquely identifies the location within the CPOs platform (and suboperator platforms). This field can never be changed, modified or renamed. 
...
|id |<<types_string_type,string>>(36) |1 |Identifier of the connector within the EVSE. Two connectors may have the same id as long as they do not belong to the same _EVSE_ object. 
...
|tariff_ids |<<types_string_type,string>>(36) |* |Identifiers of the current valid charging tariffs. Multiple tariffs are possible, but each require a unique Tariff.type, duplicates not allowed. +
...
|latitude |<<types_string_type,string>>(10) |1 |Latitude of the point in decimal degree. Example: 50.770774. Decimal separator: "." Regex: `-?[0-9]{1,2}\.[0-9]{6}` 
|longitude |<<types_string_type,string>>(11) |1 |Longitude of the point in decimal degree. Example: -126.104965. Decimal separator: "." Regex: `-?[0-9]{1,3}\.[0-9]{6}` 
...
|session_id |<<types_string_type,string>>(36) |yes |Session.id of the Session on which these Charging Preferences are to be set.
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the CPO requesting this GET to the eMSP system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the CPO requesting this GET to the eMSP system. 
|session_id |<<types_string_type,string>>(36) |yes |id of the Session object to get from the eMSP system. 
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the CPO requesting this PUT to the eMSP system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the CPO requesting this PUT to the eMSP system. 
|session_id |<<types_string_type,string>>(36) |yes |id of the new or updated Session object. 
|===
...
|id |<<types_string_type,string>>(36) |1 |The unique id that identifies the session in the CPO platform. 
...
|token_uid |<<types_string_type,string>>(36) |1 |UID of the <<mod_tokens_token_object,Token>> used to start the Charging Session, combination: Token.UID and Token.type, uniquely identifies a <<mod_tokens_token_object,Token>>.
|token_type |<<mod_tokens_tokentype_enum,TokenType>> |1 |Type of the <<mod_tokens_token_object,Token>> used to start the Charging Session, combination: Token.UID and Token.type, uniquely identifies a <<mod_tokens_token_object,Token>>.
|auth_method |<<mod_cdrs_authmethod_enum,AuthMethod>> |1 |Method used for authentication.
|location_id |<<types_string_type,string>>(39) |1 |Location.id of the Location object of this CPO, on which the Charging Session is ongoing.
|evse_uid |<<types_string_type,string>>(39) |1 |EVSE.uid of the EVSE of this Location on which the Charging Session is ongoing.
|connector_id |<<types_string_type,string>>(36) |1 |Connector.id of the Connector of this Location the Charging Session is ongoing.
...
The CDR object is different from the <<mod_sessions_session_object,Session>> object. The Session contains a lot of references to other information.
For the Session that works, if information changes during or after a Charging Session, that is not a problem.
For a CDR changing information after a Charging Session has ended, is a problem: What if the address of a <<mod_locations_location_object,Location>> is changed.
The Driver will not recognize that he has charged at that address. <<mod_tokens_token_object,Token>> auth_id might change, this should never impact an existing CDR.
...
|id |<<types_cistring_type,CiString>>(36) |1 |Uniquely identifies the CDR within the CPOs platform (and suboperator platforms). 
...
|token |<<mod_tokens_token_object,Token>> |1 |Token used to start this Charging Session.
|auth_method |<<mod_cdrs_authmethod_enum,AuthMethod>> |1 |Method used for authentication. 
...
|total_cost |<<types_number_type,number>> |1 |Total cost (excluding VAT) of this transaction in the specified currency.
|total_cost_incl_vat |<<types_number_type,number>> |1 |Total cost (including VAT) of this transaction in the specified currency.
...
|country |<<types_string_type,string>>(3) |1 |ISO 3166-1 alpha-3 code for the country of this location.
|coordinates |<<mod_locations_geolocation_class,GeoLocation>> |1 |Coordinates of the location.
|evse_uid |<<types_string_type,string>>(39) |1 |Uniquely identifies the EVSE within the CPOs platform (and suboperator platforms). For example a database unique ID or the "EVSE ID". This field can never be changed, modified or renamed. This is the 'technical' identification of the EVSE, not to be used as 'human readable' identification, use the field: evse_id for that.
|evse_id |<<types_string_type,string>>(48) |1 |Compliant with the following specification for EVSE ID from "eMI3 standard version V1.0" (http://emi3group.com/documents-links/[http://emi3group.com/documents-links/]) "Part 2: business objects.".
|connector_id |<<types_string_type,string>>(36) |1 |Identifier of the connector within the EVSE.
...
|tariff_id |<<types_string_type,string>>(36) |? |Uniquely identifier of the tariff that is relevant for this ChargingPeriod. If not provided, no tariff is relevant during this period.
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the CPO requesting this PUT to the eMSP system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the CPO requesting this PUT to the eMSP system. 
|tariff_id |<<types_string_type,string>>(36) |yes |Tariff.id of the Tariff object to retrieve. 
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the CPO requesting this PUT to the eMSP system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the CPO requesting this PUT to the eMSP system. 
|tariff_id |<<types_string_type,string>>(36) |yes |Tariff.id of the (new) Tariff object (to replace). 
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the CPO requesting this PUT to the eMSP system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the CPO requesting this PUT to the eMSP system. 
|tariff_id |<<types_string_type,string>>(36) |yes |Tariff.id of the Tariff object to delete. 
|===
...
|id |<<types_string_type,string>>(36) |1 |Uniquely identifies the tariff within the CPOs platform (and suboperator platforms). 
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the eMSP requesting this GET from the CPO system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the eMSP requesting this GET from the CPO system. 
|token_uid |<<types_string_type,string>>(36) |yes |Token.uid of the Token object to retrieve. 
|===
...
|country_code |<<types_string_type,string>>(2) |yes |Country code of the eMSP sending this PUT request to the CPO system. 
|party_id |<<types_string_type,string>>(3) |yes |Party ID (Provider ID) of the eMSP sending this PUT request to the CPO system. 
|token_uid |<<types_string_type,string>>(36) |yes |Token.uid of the (new) Token object (to replace). 
|===
...
|token_uid |<<types_string_type,string>>(36) |yes |Token.uid of the Token for which this authorization is. 
...
|uid |<<types_string_type,string>>(36) |1 |Identification used by CPO system to identify this token. Currently, in most cases: type=RFID, this is the RFID hidden ID as read by the RFID reader, but that is not a requirement.
|type |<<mod_tokens_tokentype_enum,TokenType>> |1 |Type of the token 
|auth_id |<<types_string_type,string>>(36) |1 |Uniquely identifies the EV Driver contract token within the eMSP's platform (and suboperator platforms). Recommended to follow the specification for eMA ID from "eMI3 standard version V1.0" (http://emi3group.com/documents-links/[http://emi3group.com/documents-links/]) "Part 2: business objects." 
|visual_number |<<types_string_type,string>>(64) |? |Visual readable number/identification as printed on the Token (RFID card), might be equal to the auth_id. 
...
|location_id |<<types_string_type,string>>(39) |1 |Unique identifier for the location. 
|evse_uids |<<types_string_type,string>>(39) |* |Unique identifier for EVSEs within the CPO's platform for the EVSE within the the given location. 
|connector_ids |<<types_string_type,string>>(36) |* |Identifies the connectors within the given EVSEs. | 
|===
...
|reservation_id |int |1 |Reservation id, unique for this reservation. If the Charge Point already has a reservation that matches this reservationId the Charge Point will replace the reservation.
|location_id |<<types_string_type,string>>(39) |1 |Location.id of the Location (belonging to the CPO this request is send to) for which to reserve an EVSE.
|evse_uid |<<types_string_type,string>>(39) |? |Optional EVSE.uid of the EVSE of this Location if a specific EVSE has to be reserved.
|===
...
|location_id |<<types_string_type,string>>(39) |1 |Location.id of the Location (belonging to the CPO this request is send to) on which a session is to be started.
|evse_uid |<<types_string_type,string>>(39) |? |Optional EVSE.uid of the EVSE of this Location on which a session is to be started.
|===
...
|session_id |<<types_string_type,string>>(36) |1 |Session.id of the Session that is requested to be stopped.
|===
...
|location_id |<<types_string_type,string>>(39) |1 |Location.id of the Location (belonging to the CPO this request is send to) of which it is requested to unlock the connector. 
|evse_uid |<<types_string_type,string>>(39) |1 |EVSE.uid of the EVSE of this Location of which it is requested to unlock the connector. 
|connector_id |<<types_string_type,string>>(36) |1 |Connector.id of the Connector of this Location of which it is requested to unlock. 
|===
...
| Minimal / +
Minimal
|Replace `auth_id` with `token_uid` and `token_type`. `auth_id` alone could not be used to uniquely identify a <<mod_tokens_token_object,Token>>.
