...
* Improvements:
[disc]
** <<mod_cdrs_cdr_object,CDRs>>: Credit CDRs, VAT, Session_id, AuthorizationReference, CdrLocation, CdrToken
...
|date_from |Only return objects that have `last_updated` after or equal to this Date/Time (inclusive).
|date_to |Only return objects that have `last_updated` up to this Date/Time, but not including (exclusive).
|offset |The offset of the first object returned. Default is 0 (the first object).
|limit |Maximum number of objects to GET. The server might decide to return fewer objects, either because there are no more objects, or the server limits the maximum number of objects to return. This is to prevent, for example, overloading the system.
|===
...
|X-Limit |(Custom HTTP Header) Maximum number of objects that the server can return. Note that this is an upper limit, if there are not enough remaining objects to return, fewer objects than this upper limit number will be returned, X-Limit SHALL then still show to upper limit, not the amount of objects returned.
|===
...
When development of OCPI was started, it was designed for peer-to-peer communication between CPO and eMSP.
This has advantages, but also disadvantages. Having to setup and maintain OCPI connections to a lot of parties requires more effort then doing it for only a couple of connections.
...
Some parties are themselves CPO and host CPO services for others, but other parties are them selves not a CPO or other role in the EV charging landscape, but do
provide service to CPOs/eMSPs etc.


===== Message Routing Headers
When OCPI is used to communicate to/from a Platform or via a Hub (which is almost always the case)
the following four HTTP headers are to be added to any request/response to allow message to be routed.

When implementing OCPI these four headers SHALL be implemented.
This does not mean they always have to be present in all request.
...
image::images/sd_put_boardcast.svg[OCPI Sequence Diagram of a broadcast PUT from a CPO to multiple eMSPs]
...
and calls the GET method on a Sender interface of a module.

The Hub then can then combine objects from different connected parties and return them to the client.
...
.Example sequence diagram of Boardcast Push from one Platform to another Platform via a Hub
image::images/sd_platform_hub_platform.svg[Example sequence diagram of Boardcast Push from one Platform to another Platform via a Hub]
...
When OCPI Objects are send via Hubs, the `last_updated` fields SHALL NOT be updated by the Hub.
...
|token |<<types_string_type,string>>(64) |1 |The credentials token for the other party to authenticate in your system. (Not encoded in Base64 or any other encoding)
...
|tariff_ids |<<types_cistring_type,CiString>>(36) |* |Identifiers of the currently valid charging tariffs. Multiple tariffs are possible, but each requires a unique Tariff.type. Duplicates are not allowed. +
When preference-based smart charging is supported, one tariff for every possible <<mod_sessions_profile_type_enum,ProfileType>> should be provided. These tell the user about the options they have at this Connector, and what the tariff is for every option. +
...
see <<mod_cdrs_cdr_object,CDR description>> for more information.
...
|end_date_time |<<types_datetime_type,DateTime>> |? |The timestamp when the session was completed/finished, charging might have finished before the session ends, for example: EV is full, but parking cost also has to be payed.
...
|end_date_time |<<types_datetime_type,DateTime>> |1 |The timestamp when the session was completed/finished, charging might have finished before the session ends, for example: EV is full, but parking cost also has to be payed.
...
|AUTH_REQUEST |Authentication request has been send to the eMSP.
|COMMAND |Command like <<mod_commands_startsession_object,StartSession>> or <<mod_commands_reservenow_object,ReserveNow>> used to start the Session, the Token provided in the Command used as authorization.
...
|url                     | <<types_cistring_type,CiString>>(512) |? |Link to portal where an EV driver can check the data from a charging session. (TODO: better description)
|===
...
|Alfen                     |Alfen B.V. Format |
|EDL40 E-Mobility Extension|eBee smart technologies implementation |
|EDL40 Mennekes            |Mennekes implementation |
|===

TODO: All Are we missing solutions? If so, please let us know so we can add them.

TODO: Editorial action item: Add contact information, e-mails have been send to the known organisations
...
|country_code |<<types_cistring_type,CiString>>(2) |yes |Country code of the CPO requesting this PUT to the eMSP system. This SHALL be the same value as the `country_code` in the Tariff object being pushed
|party_id |<<types_cistring_type,CiString>>(3) |yes |Party ID (Provider ID) of the CPO requesting this PUT to the eMSP system. This SHALL be the same value as the `party_id` in the Tariff object being pushed
...
|country_code |<<types_cistring_type,CiString>>(2) |1 |ISO-3166 alpha-2 country code of the CPO that 'owns' this Tariff.
|party_id |<<types_cistring_type,CiString>>(3) |1 |CPO ID of the CPO that 'owns' this Tariff (following the ISO-15118 standard).
...
|start_date_time |<<types_datetime_type,DateTime>> |? |The time when this tariff becomes active. Typically used for a new tariff that is already given with the location, before it becomes active. (See note below)
|end_date_time |<<types_datetime_type,DateTime>> |? |The time after which this tariff is no longer valid. Typically used when this tariff is going to be replaced with a different tariff in the near future. (See note below)
...
|min_current |<<types_number_type,number>> |? |Sum of the minimum current in A over all phases, for example 5. When the Charging Session is charging more then this amount of current,
...
|max_current |<<types_number_type,number>> |? |Sum of the maximum current in A over all phases, for example 20. When the Charging Session is charging less then this amount of current,
...
|min_power |<<types_number_type,number>> |? |Minimum power in kW, for example 5. When the Charging Session is consuming more then this amount of power,
...
|max_power |<<types_number_type,number>> |? |Maximum power in kW, for example 20. When the Charging Session is consuming less then this amount of power,
...
`{token_endpoint_url}{country_code}/{party_id}/{token_uid}?type={token_type}`
...
|token_type |<<mod_tokens_tokentype_enum,TokenType>> |no |Token.type of the Token to retrieve. Default if omitted: <<mod_tokens_tokentype_enum,RFID>>
|===
...
|token_type |<<mod_tokens_tokentype_enum,TokenType>> |no |Token.type of the Token of the (new) Token object (to replace). Default if omitted: <<mod_tokens_tokentype_enum,RFID>>
|===
...
```
TODO: Open Issue: what about RFID? RFID can also be provided locally, if the MSP requires realtime authorization, how can the CPO see if the authorization of the Charge Point is part of the ReserveNow or a local swipe of the RFID card? How long is the authorization given in the ReserveNow valid?
```
...
```
TODO: Open Issue: what about RFID? RFID can also be provided locally, if the MSP requires realtime authorization, how can the CPO see if the authorization of the Charge Point is part of the StartSession or a local swipe of the RFID card? How long is the authorization given in the StartSession valid?
```
...
There are different Smart Charging Topologies possible. Which topology is/can be used depends on the contracts between different parties.
...
the Receiver (Typically CPO) SHALL keep the Sender updated with changes to the ActiveChargingProfile of that Session, if the Receiver is aware of any changes,
...
For calculating optimum ChargingProfiles it might be useful for the eMSP or SCSP to known the ChargingProfile that the Charge Point has planned for the Session: ActiveChargingProfile.
...
==== Example of setting/updating a ChargingProfile by the Sender (typically the eMSP or SCSP)
...
The CPO send the requests to the Charge Point.
When the CPO receives a response from the Charge Point,
that result is send to the eMSP or SCSP by call the <<mod_charging_profiles_msp_post_method,POST>> method,
...
When a new <<mod_sessions_session_object,Session>> is started,
or when an update to an existing <<mod_sessions_session_object,Session>> is available,
the CPO sends the Session object to the eMSP, the eMSP forwards the <<mod_sessions_session_object,Session>> Object to the SCSP.
...
====  Example of a removing/clearing ChargingProfile send by the Sender (typically the eMSP or SCSP)
...
When the Sender wants to known the current planned charging profile for a session,
...
The CPO send the get charging profile requests to the Charge Point.
When the CPO receives a response from the Charge Point,
...
.Example of an ActiveClearChargingProfile being send by the CPO via the eMSP
image::images/sd_charging_profile_updated_via_msp.svg[Example of an ActiveClearChargingProfile being send by the CPO to the SCSP via the eMSP]
...
NOTE: duration: Use this wisely. Asking for a profile hours in advance might not be very useful.
But will use more mobile data then really useful. Duration of half hour,
one full hour can be really useful when checking the profile calculated by the Charge Point.
