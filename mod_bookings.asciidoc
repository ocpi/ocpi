...
and then can either choose to have the possible booking options or provide a list of bookable EVSE UIDs.
...
|booking_option |<<mod_bookings_booking_option_class, BookingOption>> | ? | Selected specification to charge at this Location.
...
|tariff_ids |<<types.asciidoc#types_cistring_type,CiString>>(36)| * | A list of Tariff id's relevant for this booking.
...
|access_information | <<mod_bookings_access_information_class, AccessInformation>> | * | Information needed to access the location.
...
Each bookingLocation should include either the booking_option or the evse_uid. One of them is mandatory.
...
|location_id |<<types.asciidoc#types_cistring_type,CiString>>(36) | 1 |Location.id of the Location object of this CPO, on which the reservation can be made.
|booking_option |<<mod_bookings_booking_option_class, BookingOption>> | ? |Specification that can be booked by drivers that want to charge at this Location.
|policy  |<<mod_bookings_policy_object, Policy>> | ? | The number of charging stations that are bookable at this location and if this is required.
|tariff_ids |<<types.asciidoc#types_cistring_type,CiString>>(36)| * | A list of Tariff id's.
...
|timeslot_increment | int | ? | The minimum allowed booking increment within available timeslot in minutes.
...
[[mod_bookings_access_information_class]]
==== AccessInformation _class_

[cols="4,2,1,9",options="header"]
|===
|Property |Type |Card. |Description
|method  | <<mod_bookings_access_method_enum, AccessMethod>> | 1 | If the location is not freely accessible, how is it accessible with the AccessMethod enum.
|value   | String | ? | The value for the location access method, so for the license plate it would be: ABC12D. Or for an access code: 1224
|===

[[mod_bookings_access_method_enum]]
==== AccessMethod _enum_
...
[[mod_bookings_booking_option_class]]
==== BookingOption _class_
...
|parking_id |<<types.asciidoc#types_cistring_type,CiString>>(36) | ? | Reference to the parking identifier.
...
|connector_format |<<mod_locations_connectorformat_enum,ConnectorFormat>> | * |The format (socket/cable) of the installed connector.
|connector_types |<<mod_locations_connectortype_enum,ConnectorType>> | * |The type of the installed connector.
|power_types |<<mod_locations_powertype_enum,PowerType>> | * |The type of power provided by the EVSE.
...
|min_parking_space_length |number |? |The minimum length of the parking space, in centimeters. A value for this field should be provided unless the value of the `vehicle_types` field contains no values other than `PERSONAL_VEHICLE` or `MOTORCYCLE`.
|min_parking_space_width |number |? |The minimum width of the parking space, in centimeters. A value for this field should be provided unless the value of the `vehicle_types` field contains no values other than `PERSONAL_VEHICLE` or `MOTORCYCLE`.
...
|booking_option |<<mod_bookings_booking_option_class, BookingOption>> | ? |Selected specification to charge at this Location.
|location_id |<<types.asciidoc#types_cistring_type,CiString>>(36) | 1 |Location.id of the Location object of this CPO, on which the reservation can be made.
|booking_location_id | <<types.asciidoc#types_cistring_type,CiString>>(36) | 1 | The unique id that identifies the BookingLocation in the CPO platform.
|tokens |<<mod_bookings_booking_token_object,BookingTokens>> | * | Token(s) that can be used to utilise the booking.
|access_information | <<mod_bookings_access_information_class,AccessInformation>> | * | Information needed to access the location.
|period |<<mod_bookings_period_class, Period>> |1 | The period for this booking.
|authorization_reference | <<types.asciidoc#types_cistring_type,CiString>>(36) | 1 | Authorization reference for the relevant Session and CDR.
...
|supported_access_methods | <<mod_bookings_access_method_enum, AccessMethod>> | + | What is needed to access the location.
...
|min_booking_duration | number | ? | Minimum booking duration in minutes.
|max_booking_duration | number | ? | Maximum booking duration in minutes.
...
[[mod_bookings_period_class]]
==== Period _class_

[cols="4,2,1,9",options="header"]
|===
|Property |Type |Card. |Description
|start_date_time | <<types.asciidoc#types_datetime_type,DateTime>> |1 | Start time of this timeslot.
|end_date_time | <<types.asciidoc#types_datetime_type,DateTime>> |1 | End time of this timeslot.
|===

[[mod_bookings_policy_object]]
==== Policy _class_
