...
No structure defined. This is open to the eMSP to define, the URL is provided to the CPO by the eMSP in the POST to the CPO interface.
...
So if the token is Whitelist: NEVER or ALLOWED_OFFLINE, the CPO will need to do a <<mod_tokens.asciidoc#mod_tokens_real-time_authorization,realtime authorization>> at the eMSP.
