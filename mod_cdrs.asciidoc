...
|booking_id |<<types.asciidoc#types_cistring_type,CiString>>(36) |? |Unique ID of the Booking for which this cdr can also be sent. Is only allowed to be omitted when the Session was reserved.
...
|RESERVATION_TIME | | Time during this  <<mod_cdrs_chargingperiod_class,ChargingPeriod>> Charge Point has been reserved time, defined in hours default step_size multiplier is 1 second.
|RESERVATION_EXPIRES| | Time during this  <<mod_cdrs_chargingperiod_class,ChargingPeriod>> Charge Point has been reserved and not yet been in use for this customer: defined in hours, default step_size multiplier is 1 second.
|RESERVATION_OVERTIME | | Time after the reservation during a <<mod_cdrs_chargingperiod_class,ChargingPeriod>> that the session continues after the reserved time slot, defined in hours, default step_size multiplier is 1 second.
