...
The CDR object is different from the <<mod_sessions#mod_sessions_session_object,Session>> object. The Session contains a lot of references to other information.
For the Session that works, if information changes during or after a Charging Session, that is not a problem.
For a CDR changing information after a Charging Session has ended, is a problem: What if the address of a <<mod_locations#mod_locations_location_object,Location>> is changed.
