...
[[mod_cdrs_invoice_reconciliation_sender_interface]]
==== Invoice Reconciliation Sender Interface
...
`+https://www.server.com/ocpi/cpo/2.2.1/cdrs/invoicereconciliation/?date_from=2019-01-28T12:00:00&date_to=2019-01-29T12:00:00+`

`+https://ocpi.server.com/2.2.1/cdrs/invoicereconciliation/?offset=50+`

`+https://www.server.com/ocpi/2.2.1/cdrs/invoicereconciliation/?date_from=2019-01-29T12:00:00&limit=100+`

`+https://www.server.com/ocpi/cpo/2.2.1/cdrs/invoicereconciliation/?offset=50&limit=100+`
...
|<<mod_cdrs_invoice_reconciliation_sender_get_method,GET>> |Fetch InvoiceReconciliationRecords last updated (which in the current version of OCPI can only be the creation Date/Time) between the `{date_from}` and `{date_to}` (<<transport_and_format.asciidoc#transport_and_format_pagination,paginated>>).
|POST |n/a
|PUT |n/a
|PATCH |n/a
|DELETE |n/a
...
[[mod_cdrs_invoice_reconciliation_sender_get_method]]
===== *GET* Method
...
|===
|Datatype |Card. |Description
|<<mod_cdrs_invoice_reconciliation_record_object,InvoiceReconciliationRecord>> |* |List of InvoiceReconciliationRecords.
|===

[[mod_cdrs_invoice_reconciliation_receiver_interface]]
==== Invoice Reconciliation Receiver Interface
...
`+https://www.server.com/ocpi/2.2.1/cdrs/invoicereconciliation+`

[cols="2,12",options="header"]
|===
|Method |Description
|<<mod_cdrs_invoice_reconciliation_receiver_get_method,GET>> |Retrieve an existing InvoiceReconciliationRecord.
|<<mod_cdrs_invoice_reconciliation_receiver_post_method,POST>> |Send a new InvoiceReconciliationRecord.
|<<mod_cdrs_invoice_reconciliation_receiver_put_method,PUT>> | Update an existing InvoiceReconciliationRecord.
|PATCH |n/a
|<<mod_cdrs_invoice_reconciliation_receiver_delete_method,DELETE>> | Delete an existing InvoiceReconciliationRecord.
|===

[[mod_cdrs_invoice_reconciliation_receiver_get_method]]
===== GET Method
...
|===
|Datatype |Card. |Description
|<<mod_cdrs_invoice_reconciliation_record_object,InvoiceReconciliationRecord>> |1 |Requested InvoiceReconciliationRecord object.
|===

[[mod_cdrs_invoice_reconciliation_receiver_post_method]]
===== POST Method
...
|===
|Type |Card. |Description
|<<mod_cdrs_invoice_reconciliation_record_object,InvoiceReconciliationRecord>> |1 |New InvoiceReconciliationRecord object.
|===

[[mod_cdrs_invoice_reconciliation_receiver_put_method]]
===== *PUT* Method
...
|===
|Type |Card. |Description
|<<mod_cdrs_invoice_reconciliation_record_object,InvoiceReconciliationRecord>> | 1 | Updated InvoiceReconciliationRecord object.
|===
...
[[mod_cdrs_invoice_reconciliation_receiver_delete_method]]
===== *DELETE* Method
...
[[mod_cdrs_invoice_reconciliation_record_object]]
...
|PARKING_TIME     | | Time in this <<mod_cdrs_chargingperiod_class,ChargingPeriod>> during which the vehicle is not requesting power. Defined in hours, default step_size multiplier is 1 second.
...
|RESERVATION_TIME | | Time in this  <<mod_cdrs_chargingperiod_class,ChargingPeriod>> during which the EVSE has been reserved and not yet been in use for this customer: defined in hours, default step_size multiplier is 1 second.
|STATE_OF_CHARGE  |Y| Current state of charge of the EV, in percentage, values allowed: 0 to 100. See note below.
|TIME             | | Time charging in this <<mod_cdrs_chargingperiod_class,ChargingPeriod>>. Defined in hours, default step_size multiplier is 1 second.
|===

NOTE: Earlier versions of the OCPI 2.3.0 specification document mistakenly defined PARKING_TIME as "Time during this <<mod_cdrs_chargingperiod_class,ChargingPeriod>> not charging". Under that definition, drivers would be exposed to penalizing loitering fees not only when they leave their vehicle in a charging session after it has been fully charged, but also when the EVSE is not offering energy to the vehicle while the vehicle is still requesting power. In practice, PARKING_TIME was already used according to the new definition.
...
|plain_data  |<<types.asciidoc#types_string_type,string>>(5000) |1 |The un-encoded string of data. The format of the content depends on the EncodingMethod field.
...
NOTE: Earlier releases of the OCPI 2.3.0 specification documentation mistakenly gave a maximum length of 512 for the `plain_data` field of the SignedValue class.
