...
|end_date_time |<<types.asciidoc#types_datetime_type,DateTime>> |1 |Stop timestamp of the charging session.
...
|COMMAND |Command like <<mod_commands.asciidoc#mod_commands_startsession_object,StartSession>> or <<mod_commands.asciidoc#mod_commands_reservenow_object,ReserveNow>> used to start the Session, the Token provided in the Command used as authorization.
...
|MAX_CURRENT |Sum of the maximum current over all phases, reached during a charging session: defined in A (Ampere)
|MIN_CURRENT |Sum of the minimum current over all phases, reached during a charging session: defined in A (Ampere)
|MAX_POWER |Maximum power reached during a charging session: defined in kW (Kilowatt)
|MIN_POWER |Minimum power reached during a charging session: defined in kW (Kilowatt)
