...
TODO add text to explain supported profile_type can found via tariffs, add to tariffs.

[cols="3,2,1,10",options="header"]
|===
|Property |Type |Card. |Description

|profile_type |<<mod_sessions_profile_type_enum,ProfileType>> |1 |The time when the session became active.
|departure |<<types.asciidoc#types_datetime_type,DateTime>> |1 |Expected departure. Driver has given this datetime as expected departure moment, which does not mean that that will be the real departure time.
|energy |<<types.asciidoc#types_number_type,number>> |1 |Requested amount of energy in kWh. EV drivers wants to have this amount of energy charged.
