...
When a PUT request is received for an existing <<mod_sessions_session_object,Session>> object: the object is posted to the same URL,
...
If the request `charging_periods` list is omitted (or contains an empty list), now changes SHALL be made to the existing list of `charging_periods`.
