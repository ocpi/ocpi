...
|[[mod_locations_location_object_parking_places_field]] parking_places |<<mod_locations_parking_class,Parking>> |* |List of parking places that can be used by vehicles charging at this Location.
...
- `Parking.parking_restrictions` not used.
...
- `Parking.parking_restrictions` = `CUSTOMERS`
...
- `Parking.parking_restrictions` = `CUSTOMERS` may still be useful so a support desk can also tell this to a customer.
...
|parking |<<types.asciidoc#types_cistring_type,CiString>>[36] |+ |A reference to the parking space or spaces that can be used by vehicles charging at this EVSE. The strings in this field refer to <<mod_locations_parking_class,Parking>> objects from the EVSE's Location's <<mod_locations_location_object_parking_places_field,parking_places>> field by their `id` field.
|vehicle_types |<<mod_locations_vehicletype_enum,VehicleType>> |+ | The vehicle types that the EVSE is intended for and that the associated parking is designed to accomodate.
|images |<<mod_locations_image_class,Image>> |* | Links to images related to the EVSE such as photos or logos. At least one photograph should be provided if the value of vehicle_types includes the `DISABLED` vehicle type.
...
NOTE: The Plug and Charge related capabilities are meant to signal that a Driver can indeed use ISO 15118 Plug and Charge at this EVSE. Mere support for a standard by the charging hardware is not enough to warrant the presence of these capabilities.
...
|direction | <<mod_locations_parkingdirection_enum,ParkingDirection>> |? | The direction in which the vehicle is to be parked next to this EVSE.
...
|restricted_to_type |boolean |1 |Whether it is forbidden for vehicles of a type not listed in `vehicle_types` to park at this EVSE, even if they can physically park there safely.
|parking_restrictions |<<mod_locations_parkingrestriction_enum,ParkingRestriction>> |* |All applicable restrictions on who can park at this EVSE, apart from those related to the vehicle type.
