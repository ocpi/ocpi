...
|tariff_ids |<<types.asciidoc#types_cistring_type,CiString>>(36) |* |Identifiers of the currently valid charging tariffs.
                    Multiple tariffs are possible, but only one of each <<mod_tariffs#mod_tariffs_tariff_object,Tariff.type>> can be active at the same time.
                    Tariffs with the same type are only allowed, if they are not active at the same time:
                    <<mod_tariffs#mod_tariffs_tariff_object,start_date_time>> and <<mod_tariffs#mod_tariffs_tariff_object,end_date_time>> period not overlapping. +
