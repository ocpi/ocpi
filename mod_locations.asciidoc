...
|address |<<types.asciidoc#types_string_type,string>>(255) |1 |Street/block name and house number if available.
...
|state |<<types.asciidoc#types_string_type,string>>(45) |? |State or province of the location, only to be used when relevant.
...
NOTE: Earlier releases of the OCPI 2.3.0 specification documentation mistakenly gave maximum lengths of 45 and 20 for the  `address` and `state` fields of the Location object.
...
|evse_id |<<types.asciidoc#types_cistring_type,CiString>>(48) |? |Compliant with the following specification for EVSE ID: "E-mobility ID-codes: the purpose of IDs, ID usage and ID format" (https://evroaming.org/contract-evse-ids/[https://evroaming.org/contract-evse-ids/]). Optional because: if an `evse_id` is to be re-used in the real world, the `evse_id` can be removed from an EVSE object if the `status` is set to `REMOVED`.
