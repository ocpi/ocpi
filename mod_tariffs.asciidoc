...
|[[mod_tariffs_preauthorize_amount_field]] preauthorize_amount | <<types_number_type,number>> |? |The amount that a Payment Terminal Provider should preauthorize when handling card payment for a Session with this Tariff.
...
|===
| Dimension | Quantity | Price ex VAT | Cost ex VAT
| Charging time | 12 minutes | 2.40 per hour | 0.48
| Time | 15 minutes | 1.00 per hour | 0.25
| Total | | | 0.73
|===
...
[[mod_tariffs_booking_restriction_type]]
==== BookingRestrictionType _enum_
...
|booking |<<mod_tariffs_booking_restriction_type,BookingRestrictionType>> |? |When this field is present, the TariffElement describes booking costs.
A booking can only have: <<mod_tariffs_tariffdimensiontype_enum,FLAT>> and <<mod_tariffs_tariffdimensiontype_enum,TIME>> TariffDimensions.
|===
