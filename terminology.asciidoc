...
|EV    |Electric Vehicle.
|EVSE  |Electric Vehicle Supply Equipment. It is considered as an independently operated and managed part of a Charge Point that can deliver energy to one EV at a time.
...
|eMSP  |e-Mobility Service Provider. It gives EV drivers access to charging services.
...
OCPI 2.0/2.1.1 had a rigorous definition of roles: only CPO and eMSP. But this is rare in the real world; there are almost no parties that are strictly CPO or eMSP and have their Platform. In the real world, lots of
parties provide service to CPOs that are not running their Platform.
A lot of CPOs are also eMSP. With OCPI 2.1.1 and earlier, that meant setting up an OCPI connection per role.
...
The Platform itself is not a role. The Platform provides services for one or more roles.

Examples of platforms:

* A pure CPO: Not providing services to other CPOs, not being an eMSP. Running its software that connects via OCPI. +
  It is defined in OCPI as a Platform with 1 CPO role, the CPO role of that company.
* A company with a cloud-based eMSP software solution offers to companies that want to be eMSP, but don't want to host/run their software. +
  It is a platform that has some eMSP roles, one for each eMSP the company is providing services for.
  Not for this company itself because the company itself is not an eMSP.
* A Company that operates public Charge Points and provides eMSP service to EV drivers, running their software platform. +
  It is seen in OCPI as a platform with two roles: CPO and eMSP for this company.
* If one of the companies above starts to offer their service to other CPOs and eMSP, it is in OCPI still seen as one Platform.
  This Platform then provides multiple CPO and eMSP roles.
* A Roaming Hub is, in OCPI terms, also a Platform; another OCPI Platform can connect to it. Most Roamings Hubs only have one role: Hub.
...
|Platform | Software that provides services via OCPI.
            A platform can provide service for a single eMSP or CPO or multiple CPOs or eMSPs.
            It can even provide services for both eMSPs and CPOs simultaneously. A Hub is also an OCPI Platform; most only have one role: Hub.
...
In OCPI it is advised to use eMI3 compliant names for Contract IDs and EVSE IDs. The provider and the operator name is essential here,
to target the right provider or operator, they need to be known upfront, at least between the cooperating parties.

An issuing authority is mentioned in several standards that will keep a central registry of known Providers and Operators.
...
The list of all registered organisations can be found on their https://idro.hu/en/page/members[website].
...
Their https://www.realschemes.org.uk/ev-roam[website] shows the complete list of currently registered organisations.
...
* _Connector_ is a specific socket or cable available for the EV to use.
* _EVSE_ is the part that controls the power supply to a single EV in a single session. An EVSE may provide multiple connectors, but only one of these can be active simultaneously.
...
A Location is typically the exact Location of one or more EVSEs, but it can also be the entrance of a parking garage or a gated community. It is up to the CPO to use whatever makes sense in a specific situation. Once arrived at the Location, any further instructions to reach the EVSE from the Location are stored in the EVSE object (such as the floor number, visual identification or manual instructions).

[[terminology_variable_names]]
=== Variable names

To prevent issues with capitals in variable names, the naming in JSON is not CamelCase but snake_case. All variables are lowercase and include an underscore for whitespace.
...
|? |An optional object. If not set, it might be `null`, or the field might be omitted. When the field is set to null or omitted and has a default value, the value is the default value. |Object
|1 |Required object. |Object 
|* |A list of zero or more objects. It might be `null`, `[]`, or the field might be omitted if empty. |[Object] 
...
When a new version of OCPI is implemented, the data exchanged via the old version does not have to be available via the more recent version of OCPI.
Hence, the Version end-point will probably have different end-points perversion. So when an object is stored with a URL that contains a version,
