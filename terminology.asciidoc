...
|NAP   |Provides a national Database with all (public) Charging Locations, information can be sent and retrieved from the NAP (that makes it different from a typical NSP).
...
Some of these roles can be combined in one company. A Platform can provide service for multiple CPOs or eMSP,
...
there are almost no parties that are strict CPO or eMSP and have their own platform. In the real world, lots of
parties provide service to CPOs that are not running their own platform.
A lot of CPOs are also eMSP. With OCPI 2.1.1 and earlier that mend having to set up an OCPI connection per role.
