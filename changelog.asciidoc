...
=== Changes between Booking-1.0 and Booking-1.1
* Updated examples and documentation
* Removed `LICENSE_PLATE` value from <<mod_tokens_tokentype_enum, TokenType>>
* Object descriptions
** Booking
*** renamed `bookable_parking_option` to `booking_option`
*** moved `evse_uid`, `connector_id` and `parking_id` to BookingOption
*** renamed `access_methods` to `access_information`
*** renamed `tariff_id` to `tariff_ids`
** BookingLocation
*** moved `evse_uid` and `connector_id` to BookingOption type
*** changed type of `bookable_parking_options` from a list to a single object
*** changed type of `booking_terms` from a list to a single object
*** renamed `bookable_parking_options` to `booking_option`
*** renamed `bookable` to `policy`
*** renamed `tariff_id` to `tariff_ids`
** Calendar
*** renamed `step_size` to `timeslot_increment`
* Data types
** AccessInformation (renamed AccessMethod)
** AccessMethod (renamed LocationAccess)
** BookingOption (Renamed from BookableParkingOption)
*** added `evse_uid`, `connector_id`, and `parking_id`
*** added `connector_types` and `power_types`
*** removed `restricted_to_type`
*** changed type of `evse_position` from a single object to a list
*** changed cardinality of `vehicle_types` from a list of at least one object to a list of zero or more objects
*** changed cardinality of `format` from required to optional
*** renamed `parking_space_length` to `min_parking_space_length`
*** renamed `parking_space_width` to `min_parking_space_width`
** BookingRequest
*** added `booking_location_id`
*** renamed `bookable_parking_option` to `booking_option`
*** moved `evse_uid` and `connector_id` a to BookingOption
*** changed type of `period` from Timeslot to Period
** BookingTerms
*** Added `min_booking_duration` and `max_booking_duration`
** BookingToken
*** removed `license_plate`
** Period (new type)
*** added `start_date_time` and `end_date_time`
** Policy (renamed Bookable)
** Timeslot
*** renamed `start_from` to `start_date_time`
*** renamed `end_from` to `end_date_time`
