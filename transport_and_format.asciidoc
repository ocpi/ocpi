...
<<credentials.asciidoc#credentials_credentials_endpoint,'Credentials tokens'>> are exchanged via the <<mod_tokens.asciidoc#mod_tokens_tokens_module,credentials module>>.
These are different 'tokens' then the <<,Tokens>> exchanged via the <<,Token Module>>: Tokens used by drivers to authorize charging.
...
If the header is missing or the credentials token doesn't match any known party then the server MUST respond with a HTTP `401 - Unauthorized` status code.
