...
When OCPI is used to via a Hub or to a virtual CPO/eMSP the following 4 HTTP headers have to be used, in requests and responses.
...
.Example sequence diagram of a GET from a CPO to an MSP.
...
When a party responses to a request (via a hub), and it was a request with a to field, the from field might be omitted. The requesting party knows to which party the request was send.
It might be good practice to always provide a FROM address in a response, just to be complete and to give a requesting party to change to validate if the request was routed correctly by the Hub.
...
To send a Broadcast push, the client uses to Hubs party-id and country-code in the HTTP headers as TO address.
The Hub parses the request and send a response to the client, with optionally its party-id and country-code in the FROM address HTTP headers.
The Hub then send to push to any client implementing the applicable module, using its own party-id and country-code as the FROM address.
The client receiving a push from a Hub (with the Hubs FROM address) will respond to this push with the Hubs party-id and country-code in the TO address HTTP headers.

GET should not be allowed in combination with broadcast. If the Requesting party wants to GET information of which it does not know the receiving party, to Open request should be used. (see below)
...
(TODO Anybody have a better name for this?)
For an intelligent Hub or routing to unknown destination.
When a Hub has the intelligence to route messages, based on the content of the request,
or the requesting party does not know the destination of a request, the TO headers should be ommitted in the request towards a hub.
...
The following tables shows which headers are required/optional to be used, and which TO/FROM IDs need to be used.
...
This table contains the description of which header are required to be used for which message when a request is to be broadcasted by the hub.
[cols="8,10,8,8,10",options="header"]
|===
| Name | Route | TO Header | FROM Header | Description
...
This table contains the description of which header are required to be used for which message when a request to be routed by the Hub itself.
