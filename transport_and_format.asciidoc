...
|limit |The maximum number of objects to GET. The server might decide to return fewer objects, either because there are no more objects, or the server limits the maximum number of objects to return. This is to prevent, for example, overloading the system.
|===
...
|X-Total-Count |(Custom HTTP Header) The total number of objects available in the server system that match the given query
...
|X-Limit |(Custom HTTP Header) The maximum number of objects that the server can return.
...
The <<credentials.asciidoc#credentials_credentials_object,country_code>>
and <<credentials.asciidoc#credentials_credentials_object,party_id>>
of the other party are received during the <<credentials.asciidoc#credentials_credentials_endpoint,credentials>> handshake,
...
then given during the <<credentials.asciidoc#credentials_credentials_endpoint,credentials>> handshake,
