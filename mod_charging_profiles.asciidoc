...
The ActiveChargingProfile might differ from ChargingProfile requested via OCPI. There might be other limiting factors being taken into account by the CPO and or Charging Station,
...
The CPO will then ask the Charging Station for the planned ActiveChargingProfile. When that is received it is forwarded to the URL given by the eMSP or SCSP.
...
that result is send to the eMSP or SCSP by call the <<mod_charging_profiles_msp_post_methodm,POST>> method,
...
The CPO send the requests to the Charge Point.
When the CPO receives a response from the Charge Point,
that result is send to the eMSP by call the <<mod_charging_profiles_msp_post_methodm,POST>> method,
