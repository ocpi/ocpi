...
|<<mod_charging_profiles_msp_put_method,PUT>> |CPO can send an updated composite schedule when other inputs have made changes to existing schedule.
        When the CPO sends a update schedule to the EVSE, for an other reason then the eMSP, the CPO SHALL post an update to this interface.
...
As the eMSP interface is called by the CPO on the URL given `response_url` in the eMSP request to the CPO interface,
...
No structure defined. This is open to the eMSP to define, the URL is provided to the CPO by the eMSP.
