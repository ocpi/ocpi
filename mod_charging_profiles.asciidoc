...
==== Example of a SetChargingProfile send by the Sender (typically the eMSP or SCSP)
...
The eMSP or SCSP calculates a Charging Profile and sends it to the CPO by calling the Charging Profiles <<mod_charging_profiles_cpo_put_method,PUT>> interface.

The CPO responds to the eMSP or SCSP, the response body will contain the result of the request,
acknowledging the request was understood an can be forwarded to the Charge Point.

The CPO send the requests to the Charge Point.
When the CPO receives a response from the Charge Point,
that result is send to the eMSP or SCSP by call the POST method, on the URL provided by the eMSP of SCSP in the <<mod_charging_profiles_cpo_put_method,PUT>> request.

.Example of a SetChargingProfile.
image::images/sd_charging_profile_set.svg[Example of a SetChargingProfile]


==== Example of a SetChargingProfile send by the SCSP via the eMSP
...
When a new <<mod_sessions.asciidoc#mod_sessions_session_object,Session>> is started,
or when an update to an existing <<mod_sessions.asciidoc#mod_sessions_session_object,Session>> is available,
the CPO sends the Session object to the eMSP, the eMSP forwards the <<mod_sessions.asciidoc#mod_sessions_session_object,Session>> Object to the SCSP.
The SCSP calculates a Charging Profile and sends it to the eMSP by calling the Charging Profiles <<mod_charging_profiles_cpo_put_method,PUT>> interface of the eMSP,
the eMSP forwards it to the CPO by calling the Charging Profiles <<mod_charging_profiles_cpo_put_method,PUT>> interface of the CPO.

The CPO responds to the eMSP, the response body will contain the result of the request,
acknowledging the request was understood an can be forwarded to the Charge Point.
The eMSP forwards this reponse to the SCSP.

The CPO send the requests to the Charge Point.
When the CPO receives a response from the Charge Point,
that result is send to the eMSP by call the POST method, on the URL provided by the eMSP in the <<mod_charging_profiles_cpo_put_method,PUT>> request of the eMSP.
...
==== Example of a GetActiveChargingProfile send by the Sender (typically the eMSP or SCSP)
...
====  Example of a ClearChargingProfile send by the Sender (typically the eMSP or SCSP)

TODO


====  Example of a ClearChargingProfile send by the SCSP via the eMSP

TODO


====  Example of a UpdateActiveChargingProfile send by the Receiver (typically the CPO)

TODO


====  Example of a UpdateActiveChargingProfile send by the Receiver (typically the CPO) via the eMSP
...
|===
|Property           |Type |Card. |Description
|start_date_time    |<<types.asciidoc#types_datetime_type,DateTime>> | 1 | Date and time at which the profile becomes active. All time measurements within the profile are relative to this timestamp.
