...
The proof is in the pudding, we need implementations to prove OCPI 2.2 is mature enough to be released as FINAL.
When developers are implementing a specification they are the ones finding real-world issues, most of the times.
If they can implement OCPI 2.2 and get it working with other parties, that is the proof we need.
