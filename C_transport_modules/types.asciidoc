...
|CPO |Charge Point Operator role (in case of a peer-to-peer connection to a TCPO this is the ACPO)
|CSP |Customer Service Provider role.
...
|TCPO  |Technical CPO role, SHALL only be used to connect to a ACPO (which is then using the CPO role).
|===
